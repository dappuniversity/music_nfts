...
export default function MyResales({ nftMarketplace, account }) {
  ...
  const loadMyResales = async () => {
    ...
    let filter = nftMarketplace.filters.MarketItemRelisted(null, account, null)
    let results = await nftMarketplace.queryFilter(filter)
    ...
    filter = nftMarketplace.filters.MarketItemBought(null, account, null, null)
    results = await nftMarketplace.queryFilter(filter)
    console.log(results)
          ...
          <Row xs={1} md={2} lg={4} className="g-4 py-3">
            {listedItems.map((item, idx) => (
              <Col key={idx} className="overflow-hidden">
                <audio src={item.audio} ref={el => audioRef.current[idx] = el}></audio>
                <Card>
